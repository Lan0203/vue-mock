...
for(let i = 0; i < 8; i++){
  ...
  article.push({
    ...
    actions:[
      { type: 'star-o', text: Mock.mock('@integer(1, 999)') },
      { type: 'like-o', text:  Mock.mock('@integer(1, 999)') },
      { type: 'message', text:  Mock.mock('@integer(1, 999)') },
    ],
    members: [
      {
...
for (let i = 0; i < 11; i++) {
  appList.push({
    ...
    activeUser: Mock.mock('@integer(1, 100)'),
    newUser: Mock.mock('@integer(1000, 3000)')
  })
}
export default {
