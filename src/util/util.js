...
//根据value值取label
export function selectName(sList, id) { //根据键，获取下拉框对应值
  let selectedWorkName = {};
  selectedWorkName = sList.find((item) => { //这里的sList就是上面遍历的数据源
    return item.value === id; 
    //筛选出匹配数据，是对应数据的整个对象 
  });
  return selectedWorkName.label;
}
export function JsonResultSort(name, minor){
  return function (o, p) {
      var a, b;
